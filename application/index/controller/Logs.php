...
class Logs extends Common{
    ...
    public function edit(){
        ...
        $list=db('logs')->alias('l')->join('admin a','l.uid=a.id')->join('admin m','l.to=m.id','LEFT')->field('l.*,a.name,m.name as ton')
        ->where('l.type',$t)->where('time','>= time',$time)->where('uid|to',$uid)->order('time desc')->select();
        $this->assign('t',$t);
        $this->assign('u',$user);
        $this->assign('nid',$uid);
