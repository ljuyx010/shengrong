...
<head>
    ...
    {css href="__PUBLIC__/jump/msgbox.css" /}
    {css href="__PUBLIC__/jump/sweetalert.css" /}
...
<body   style="margin:auto;width:20%;margin-top:20%;background:#F9F9F9">
    <div class="system-message">
      <!-- 成功提示 -->
        <?php if($code) {?>
            ...
            </div><div class="sa-icon sa-custom" style="display: none; width: 80px; height: 80px; background-image: url(__PUBLIC__/jump/thumbs-up.jpg);"></div><h2><?php echo(strip_tags($msg));?></h2>
            <p style="display: none;">正在跳转，等待时间： <b id="wait"><?php echo($wait);?></b></p>
            <fieldset>
    ...
    <!-- 失败提示 -->
		<?php }else{?>
          ...
          <div class="sa-placeholder"></div>
          <div class="sa-fix"></div>
        </div><div class="sa-icon sa-custom" style="display: none;"></div><h2><?php echo(strip_tags($msg));?>!</h2>
        <p style="display: none;">正在跳转，等待时间： <b id="wait"><?php echo($wait);?></b></p>
        <fieldset>
          <input type="text" tabindex="3" placeholder="">
          <div class="sa-input-error"></div>
